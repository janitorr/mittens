...
Fact-related infrastructure in the Host (data access, endpoints) SHALL be organized under `Fact/` subdirectories: `Fact/Data/` for persistence, `Fact/Endpoints/` for API definitions.

#### Scenario: Fact data access is in Fact/Data
- **WHEN** locating `FactReader`, `FactWriter`, or `FactSql`
- **THEN** they are found at `Mittens/Fact/Data/`

#### Scenario: Fact endpoints are in Fact/Endpoints
- **WHEN** locating REST or MCP endpoint definitions for Facts
- **THEN** they are found at `Mittens/Fact/Endpoints/`
...
#### Scenario: Health endpoint is not in Fact folder
- **WHEN** locating the health check endpoint
- **THEN** it is found at `Mittens/Endpoints/HealthEndpoints.cs`, not inside `Fact/`

### Requirement: Compiled EF model is in Fact/Data/Compiled
The precompiled EF Core model files SHALL reside in `Mittens/Fact/Data/Compiled/`, co-located with the data access layer they support.

#### Scenario: Compiled model is discoverable with data access code
- **WHEN** navigating the Host project
- **THEN** `AppDbContextModel.cs` and related files are found under `Fact/Data/Compiled/`
